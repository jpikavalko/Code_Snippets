Shader "Custom/Disappear" {
    Properties {
		...
		_PositionTreshold("Position treshold", Range(0.01, 100)) = 1
        [Toggle(ALPHA)] _ALPHA("No Shadows on Transparent", Float) = 0  
    }
 
    SubShader{
        Tags { "RenderTy pe" = "Transparent" }
        LOD 200
		...
		void surf (Input IN, inout SurfaceOutput o) {
			...
			half4 nSide1 = tex2D(_NoiseTex, (IN.worldPos.xy) * _NScale);
			...
			float3 dis = distance(_Position * _PositionTreshold, IN.worldPos);
